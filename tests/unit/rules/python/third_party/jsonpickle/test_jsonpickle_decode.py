...
class JsonPickleDecodeTests(test_case.TestCase):
    def setUp(self):
        super().setUp()
        self.parser = python.Python(enabled=["PRE0505"])

    def test_jsonpickle_decode_rule_meta(self):
        rule = Rule.get_by_id("PRE0505")
        self.assertEqual("PRE0505", rule.id)
        self.assertEqual("deserialization_of_untrusted_data", rule.name)
        self.assertEqual(
            "https://docs.securesauce.dev/rules/PRE0505", rule.help_url
        )
    ...
    def test_jsonpickle_decode(self):
        ...
        result = results[0]
        self.assertEqual("PRE0505", result.rule_id)
