...
class SslContextTests(test_case.TestCase):
    def setUp(self):
        super().setUp()
        self.parser = python.Python(enabled=["PRE0510"])
    ...
    def test_ssl_context_rule_meta(self):
        rule = Rule.get_by_id("PRE0510")
        self.assertEqual("PRE0510", rule.id)
        self.assertEqual("inadequate_encryption_strength", rule.name)
        self.assertEqual(
            "https://docs.securesauce.dev/rules/PRE0510", rule.help_url
        )
    ...
    def test_ssl_context_sslv2(self):
        ...
        result = results[0]
        self.assertEqual("PRE0510", result.rule_id)
