...
class JsonLoadTests(test_case.TestCase):
    ...
    def test_json_load_rule_meta(self):
        rule = Rule.get_by_id("PRE0008")
        self.assertEqual("PRE0008", rule.id)
        self.assertEqual("deserialization_of_untrusted_data", rule.name)
        self.assertEqual(
            "https://docs.securesauce.dev/rules/PRE0008", rule.help_url
        )
    ...
    def test_json_jsondecoder_decode(self):
        ...
        result = results[0]
        self.assertEqual("PRE0008", result.rule_id)
